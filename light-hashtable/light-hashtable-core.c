...
uint32_t FNV1A(char* data, int len){ //Algorithm for hash function
...
int HT_INSERT_KV(HT* ht, char* k, char* v){
	...
	for (int i = 0; i < HTSIZE; i++) {
		...
		if (!ht->items[try].occupied) { 
			...
			ht->items[try].key[KEYSIZE-1] = '\0';
			ht->items[try].value[VALSIZE-1] = '\0';
		...
		if(strncmp(ht->items[try].key, k, KEYSIZE-1) == 0){

			memcpy(ht->items[try].value, v, VALSIZE-1);
			ht->items[try].occupied = 1; 
...
int HT_OVERWRITE_K(HT* ht, char* k, char* nk){
	...
	for(int i = 0; i<HTSIZE; i++){
		...
		if(ht->items[try].occupied && strncmp(ht->items[try].key, k, KEYSIZE-1) == 0){
			memset(&ht->items[try].key, 0, KEYSIZE); // Clear the key buffer in hashtable item 
			memcpy(ht->items[try].key, nk, KEYSIZE-1); // Copy new key as raw bytes
			return 0; 
		} 
	}
	return -1;	// NOTE: This scheme exists across both overwrite operations
} 
int HT_OVERWRITE_V(HT* ht, char* v, char* nv){
	...
	for(int i = 0; i<HTSIZE; i++){
		...
		if(ht->items[try].occupied && strncmp(ht->items[try].value, v, VALSIZE-1) == 0){
			memset(&ht->items[try].value, 0, VALSIZE); 
			memcpy(ht->items[try].value, nv, VALSIZE-1); 
			return 0;
		} 
	}
	return -1;
...
int HT_GET_V_ASINT(HT* ht, char* k){
	...
	for(int i = 0; i<HTSIZE; i++){
		int try = (index + i) % HTSIZE;
		if(ht->items[try].occupied && strncmp(ht->items[try].key, k, KEYSIZE-1) == 0){
			return strtol(ht->items[try].value, NULL, 10); // Same scheme as HT_GET_V(), only with conversion for a long integer instead of char* 
		} 
	} 

	return -1;
} 
float HT_GET_V_ASFLOAT(HT* ht, char* k){
	int index = HT_HASH(k); 
